...
export const isAuthorized = ({
  ...
  if (isAuth) {
    if (!isVerifiedUser) {
      const isConfirmPage = pathname.startsWith('/auth/confirm-email');
      const isHome = pathname === '/';
      // Allow homepage even if email is not verified; redirect other routes to confirm page
      if (!isConfirmPage && !isHome) {
