...
export class AuthController {
   ...
   * @returns {Promise<{ exists: boolean }>} Whether the user exists.
   */
  @Public()
  ...
  async register(
    ...
    this.logger.log(`Checking if user exists: ${createUserDto.email}`);
    const existingUser = await this.authService.findUserByEmail(createUserDto.email);
    this.logger.log(`User exists check result:`, existingUser ? 'User found' : 'User not found');
    
    if (existingUser) {
      this.logger.log(`User already exists: ${createUserDto.email}`);
  ...
  async signIn(@Body() signInUserDto: SignInUserDto): Promise<SignInResponse> {
    this.logger.log(`Sign-in attempt for identifier: ${signInUserDto.identifier}`);
    try {
      const data = await this.authService.signIn(signInUserDto);
      const result = data.data;
      this.logger.log(`User signed in successfully: ${signInUserDto.identifier}`);
      ...
      this.logger.error(`Sign-in failed for ${signInUserDto.identifier}:`, error);
      throw error;
    }
  }
